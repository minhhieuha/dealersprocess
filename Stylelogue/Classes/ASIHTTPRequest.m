...
static NSTimeInterval defaultTimeOutSeconds = 300;
...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
		...
		[sharedQueue setMaxConcurrentOperationCount:4];//set max request
...
{
#if NSLog_REQUEST_STATUS || NSLog_THROTTLING
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if ([self connectionInfo]) {
			...
			// Check if we should have expired this connection
				...
				#if NSLog_PERSISTENT_CONNECTIONS
