...
@implementation GoodyController
...
-(id)init {
    ...
    if ((self = [super init])) {
		...
		self.navigationItem.titleView = [FontLabel titleLabelNamed:@"Fashion Loots"];
...
- (void)loadView {
	...
	currentPage = 1;
	[self.api listGoodies:20 pageNum:currentPage];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (nodeCount > 0)
	{
		...
		if (indexPath.row == nodeCount) {
			...
			cell.rate.text = @"";
			cell.stateLabel.text=@"More Loots...";
