...
@implementation GiveOpinionControllerHot
...
@synthesize currentPage, loginToolbar;
...
- (id)init {
    ...
    if ((self = [super init])) {
		...
		self.navigationItem.titleView = [FontLabel titleLabelNamed:@"Hot"];
...
- (void)loadView {
	...
	currentPage = 1;
	[self.api listHostItemOfCurrentUser:20 pageNum:currentPage];
...
{	
	...
	if ([self.entries count] == indexPath.row) {
		...
		viewDetail.navigationItem.titleView = [FontLabel titleLabelNamed:@"Hot"];
